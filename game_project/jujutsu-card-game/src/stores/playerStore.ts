...
import { BATTLE_SIZE, MAX_CREW_SIZE, SALARY_CAP } from '../data/constants';
  ...
  persist(
    (set, get) => ({
      ...
      setCurrentCrew: (crew: string[]) => {
        if (crew.length < BATTLE_SIZE || crew.length > MAX_CREW_SIZE) return false;
      ...
      addCardToCrew: (cardId: string) => {
        ...
        if (player.currentCrew.length >= MAX_CREW_SIZE) return false;
      ...
      validateAndFixCrew: () => {
        ...
        if (!needsOwnedCardsSync && currentCrew.length <= MAX_CREW_SIZE && totalSalary <= SALARY_CAP) {
        ...
        for (const cardId of currentCrew) {
          if (validatedCrew.length >= MAX_CREW_SIZE) {
